...
while flag<=n:
    ...
    mdb.models['Model-10'].materials['Material-1'].Elastic(table=((1.0, 0.3), ))
    ...
    a.seedEdgeBySize(edges=bon, size=0.15, deviationFactor=0.1, 
        constraint=FINER)
    ...
    mdb.Job(name='1', model='Model-10', description='', type=ANALYSIS, 
    atTime=None, waitMinutes=0, waitHours=0, queue=None, memory=90, 
    memoryUnits=PERCENTAGE, getMemoryFromAnalysis=True, 
    explicitPrecision=SINGLE, nodalOutputPrecision=SINGLE, echoPrint=OFF, 
    modelPrint=OFF, contactPrint=OFF, historyPrint=OFF, userSubroutine='', 
    scratch='', multiprocessingMode=DEFAULT, numCpus=4,numDomains=4 , numGPUs=0)
    mdb.jobs['1'].submit(consistencyChecking=OFF)
    
    mdb.jobs['1'].waitForCompletion()
    ...
    odbPath='E:\Abaqus\\'+job_name+'.odb'                   ##Path
    ...
    numpy.savetxt('aax'+repr(flag)+'.txt',xx,fmt='%f')
    numpy.savetxt('aay'+repr(flag)+'.txt',yy,fmt='%f')
    numpy.savetxt('aau1'+repr(flag)+'.txt',u1,fmt='%f')
    numpy.savetxt('aau2'+repr(flag)+'.txt',u2,fmt='%f')
    print(st)
